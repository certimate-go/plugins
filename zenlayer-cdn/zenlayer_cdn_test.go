...
func TestGetMetadata(t *testing.T) {
	meta, err := (&zenlayerCdnDeployer{}).GetMetadata(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigSchema(t *testing.T) {
	schema, err := (&zenlayerCdnDeployer{}).GetConfigSchema(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeploy_InvalidAccessConfig(t *testing.T) {
	_, err := (&zenlayerCdnDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
...
func TestDeploy_BranchValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			extendedConfig:   `{"deployTarget":"domain"}`,
