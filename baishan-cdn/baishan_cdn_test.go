...
func TestGetMetadata(t *testing.T) {
	meta, err := (&baishanCdnDeployer{}).GetMetadata(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigSchema(t *testing.T) {
	schema, err := (&baishanCdnDeployer{}).GetConfigSchema(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	...
	credentialFields := map[string]bool{"apiToken": true}
...
func TestDeploy_InvalidAccessConfig(t *testing.T) {
	_, err := (&baishanCdnDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
...
func TestDeploy_InvalidDeployConfig(t *testing.T) {
	_, err := (&baishanCdnDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
		AccessConfigJSON:   `{"apiToken":"test-api-token"}`,
...
func TestDeploy_BranchValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "deploy to domain without domain",
			extendedConfig:   `{"deployTarget":"domain","domainMatchPattern":"exact"}`,
...
func TestConfigParseParity(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"apiToken":"tk"}`), &access); err != nil {
		t.Fatal(err)
	}
	if access.ApiToken != "tk" {
	...
	if normalizeDomain(domainExtended.Domain) != ".example.com" {
		t.Fatalf("normalizeDomain = %q, want .example.com", normalizeDomain(domainExtended.Domain))
	}

	var certExtended extendedConfig
	if err := json.Unmarshal([]byte(`{"deployTarget":"certificate","certificateId":"123456"}`), &certExtended); err != nil {
		t.Fatal(err)
	}
	...
	if certExtended.CertificateId != "123456" {
		t.Fatalf("certificateId = %q, want 123456", certExtended.CertificateId)
	}
}
