...
func TestDeploy_EnvGated(t *testing.T) {
	fp.Parse()
	if fAccessKeyId == "" || fAccessKeySecret == "" || fApiKey == "" || fTestCertPath == "" || fTestKeyPath == "" {
		t.Skip("requires WANGSUCDNPRO_ credentials and test cert paths")
	}
	...
	t.Run("Deploy", func(t *testing.T) {
		if fDomain == "" {
			t.Skip("requires WANGSUCDNPRO_DOMAIN")
		}
		...
		if fWebhookId != "" {
			extendedConfigData["webhookId"] = fWebhookId
		}
