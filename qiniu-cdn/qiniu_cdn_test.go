...
func TestGetMetadata(t *testing.T) {
	meta, err := (&qiniuCdnDeployer{}).GetMetadata(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigSchema(t *testing.T) {
	schema, err := (&qiniuCdnDeployer{}).GetConfigSchema(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeploy_InvalidAccessConfig(t *testing.T) {
	_, err := (&qiniuCdnDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
