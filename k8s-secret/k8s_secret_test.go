...
func TestGetMetadata(t *testing.T) {
	meta, err := (&k8sSecretDeployer{}).GetMetadata(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	...
	if meta.DeployCategory != "other" {
		t.Fatalf("deploy category = %q, want other", meta.DeployCategory)
	}
...
func TestGetConfigSchema(t *testing.T) {
	schema, err := (&k8sSecretDeployer{}).GetConfigSchema(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeploy_InvalidAccessConfig(t *testing.T) {
	_, err := (&k8sSecretDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
		AccessConfigJSON:   `{not-json`,
		ExtendedConfigJSON: `{"namespace":"default","secretName":"test"}`,
	}, nil)
