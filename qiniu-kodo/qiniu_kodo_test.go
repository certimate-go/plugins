...
func TestGetMetadata(t *testing.T) {
	meta, err := (&qiniuKodoDeployer{}).GetMetadata(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigSchema(t *testing.T) {
	schema, err := (&qiniuKodoDeployer{}).GetConfigSchema(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeploy_MissingDomain(t *testing.T) {
	_, err := (&qiniuKodoDeployer{}).Deploy(context.Background(), &plugin.DeployRequest{
		AccessConfigJSON:   `{"accessKey":"ak","secretKey":"sk"}`,
		ExtendedConfigJSON: `{"bucket":"b"}`,
	}, nil)
